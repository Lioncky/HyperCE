...
void on_show_process_list_menu(HMENU hPopup) {
    ...
    Nt::EnumProcETS(+[](PSYSTEM_PROCESS_INFORMATION_NS p) {
        if (nt::wcsstr(p->ImageName.Buffer, L"svchost"))
			return true;
		if (nt::wcsstr(p->ImageName.Buffer, L"msedge"))
			return true;
		if (nt::wcsstr(p->ImageName.Buffer, L"chrome"))
			return true;
		if (nt::wcsstr(p->ImageName.Buffer, L"vcpkg"))
			return true;

        HANDLE hp = Nt::OpenProc(p->UniqueProcessId, PROCESS_QUERY_INFORMATION);
		AppendMenuW(hPop, 
            MF_STRING | (hp ? (p->UniqueProcessId == nt::pidh() ? MF_CHECKED : 0) : MF_DISABLED), 0x88000000 | (UINT)(INT_PTR)p->UniqueProcessId,
            nt::wsprint(G->bHexMode ? L"%X %s": L"%d %s", p->UniqueProcessId, p->ImageName.Buffer ? p->ImageName.Buffer : L"")
        );
		if (hp)
			nt::ntClose(hp);
        return true; 
...
void on_show_module_list_menu(HMENU hPopup) {
    ...
    nt::pmodules(+[](LDR_DATA_TABLE_ENTRY_NS* p) {
        ...
        AppendMenuW(hPopModule,
            ...
            //nt::wsprint(L"%s(%llX~%llX)", p->BaseDllName.Buffer, p->DllBase, (char*)p->DllBase + p->SizeOfImage)
...
HMENU MmviewMenu()
{
	...
	//AppendMenuW(hFunc, MF_STRING, IDM_PROC_CURRENT, L"");
	...
	//AppendMenuW(hFunc, MF_POPUP, IDM_PROC_LIST, L"进程(&L)");
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYUP:
        if (wParam == 'C' && (::GetAsyncKeyState(VK_CONTROL) & 0x8000)) {
            ...
            size_t    cnt = hi - lo + 1;
            char * pd,*p, * buf = (char*)nt::valloc(cnt << 2);
			if (!buf) break; p = buf;
	...
	{
        if (LOWORD(lParam) == 0) // get process list 
        {
			on_show_process_list_menu((HMENU)wParam);
            return 0;
        }
        if (LOWORD(lParam) == 1) // get process list 
        {
    ...
    case WM_COMMAND:
		...
		case IDM_PROC_PAUSE: {
			...
			if (state & MF_CHECKED) {
				CheckMenuItem(hMenu, IDM_PROC_PAUSE, MF_BYCOMMAND | MF_UNCHECKED);

			}
			else {
				CheckMenuItem(hMenu, IDM_PROC_PAUSE, MF_BYCOMMAND | MF_CHECKED);
